...
# Download EFF Wordlist if missing (used for hostname generation)
if [ ! -f "eff_large_wordlist.txt" ]; then
    echo "Downloading EFF wordlist for hostname generation..."
    curl -fsSL -o eff_large_wordlist.txt "https://www.eff.org/files/2016/07/18/eff_large_wordlist.txt"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to download EFF wordlist."
        exit 1
    fi
fi
...
ansible-playbook -i "$TEMP_INVENTORY" playbook.yml $ANSIBLE_ARGS \
    --extra-vars "llm_provider='$LLM_PROVIDER' llm_model='$LLM_MODEL' llm_url='$LLM_URL' llm_key='$LLM_KEY' github_token='$GITHUB_TOKEN' github_user='$GITHUB_USER' agentmail_key='$AGENTMAIL_KEY'"
